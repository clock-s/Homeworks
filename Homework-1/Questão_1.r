modee <- function(x) {
  tabela_frequencias <- table(x)
  frequencia_maxima <- max(tabela_frequencias)
  modas <- as.numeric(names(tabela_frequencias)[tabela_frequencias == frequencia_maxima])
  return(modas)
}
...
print(paste("Moda(s):", toString(moda)))
...
cat("\n--- Item 2 ---\n")
hist(lista, main = "Histograma das emissões diárias", xlab = "Emissões (unidade)")
boxplot(lista, main = "Boxplot das emissões diárias", ylab = "Emissões (unidade)")

cat("\n--- Item 3 ---\n")
quartis <- quantile(lista, probs = c(0.25, 0.50, 0.75))
Q1 <- quartis[1]
Q3 <- quartis[3]
IQR_val <- Q3 - Q1
limite_inferior <- Q1 - 1.5 * IQR_val
limite_superior <- Q3 + 1.5 * IQR_val
outliers <- lista[lista < limite_inferior | lista > limite_superior]
...
cat("\n--- Item 4 ---\n")
limite <- 25
excedentes <- sum(lista > limite)
total <- length(lista)
proporcao <- excedentes / total
percentual <- proporcao * 100
print(paste("Total de dias (observações):", total))
print(paste("Dias com emissão >", limite, ":", excedentes))
print(paste("Proporção de dias excedidos:", proporcao))
print(paste("Percentual de dias excedidos:", percentual, "%"))